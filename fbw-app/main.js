...
Vue.prototype.requestApi = function(options) {
	uni.showToast({ title: '加载中...', mask: true, icon: 'loading', duration: 10000 })
	...
	if(options.upload_file){
		...
		uni.request({
			...
			fail: () => {
				options.hide_loading || uni.hideToast();
				uni.showToast({
					title: '无法连接服务器',
					icon: 'none'
				})
			...
			success: (res) => {
				options.manually_hide_loading || options.hide_loading || uni.hideToast();
