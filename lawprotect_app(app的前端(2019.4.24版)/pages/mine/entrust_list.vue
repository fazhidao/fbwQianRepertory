...
<div style="padding: 1px 0;">
	<view class="chose_box">
		...
		<view class="borderX" style="background: #FFFFFF;"></view>
		<view class="chose_type_box" @click="is_show_types = true">
			<text v-if="!chosed_type">点击选择类型</text>
			<text v-else>已选择 {{chosed_type.title}}</text>
		</view>
	</view>
	...
	<view v-if="is_show_select_area" class="select_box absolute_center">
		...
		<view class="agreement_text agreement_text2" @click="backToAllArea()">返回全部地区</view>
	</view>
	
	<view v-if="is_show_types" class="select_bg" @click="is_show_types=false"></view>
	<view v-if="is_show_types" class="select_box absolute_center">
		<view class="select_title">案件类型</view>
		<scroll-view class="select_content_box" scroll-y>
			<view v-for="item in type_list" :key="item.id">
		...
		<view class="agreement_text agreement_text3" @click="backToAllType()">返回全部类型</view>
	</view>
...
export default{
	...
	methods:{
		getDataList(){
			...
			if(this.chosed_type){
				request_data.type_id = this.chosed_type.id;
			}
			this.requestApi({
		...
		choseType(item){
			this.type_list = this.type_list.map(el=>{
				el.chosed = item.id == el.id;
				return el;
			})
			this.chosed_type = item;
			this.is_show_types = false;
			this.getDataList('new_arr');
		},
		backToAllType(){
			this.type_list = this.type_list.map(el=>{
