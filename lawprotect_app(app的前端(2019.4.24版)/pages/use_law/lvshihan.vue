...
<div>
	<view class="make_contract_info" style="margin-top: 50px;">
		<view style="min-height: 200upx;display: flex;" @click="focusTextarea()">
			<text>律师函内容：</text>
			<textarea style=" position: relative;left:40upx;" class="contract_type_textarea" v-model="content" placeholder="请详细描述律师函具体内容" :auto-height="true" :focus="focus" @blur="focus=false" />
		</view>
...
export default{
	...
	methods: {
		...
		showPayment(id){
			let passback_params = JSON.stringify({ make_contract: 4, id });
			...
			global.payment_title = '律师函';
			
			this.goView('/pages/payment/chose_type?goods_type=lvshihan_make');
		},
		showSuccess(){
			this.content = '';
			this.selected_type = '';
